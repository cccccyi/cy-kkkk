...
def audience_registration_ids(title, content, uniqCode, registration_ids):
    ...
    ios_msg = jpush.ios(alert={"title": title, "body": content}, badge=0, extras={'newsid': uniqCode})
    ...
    try:
        with connection.cursor() as cursor:
            ...
            for row in rows:
                ...
                response = audience_registration_ids(row['title'], row['detail_content'], row['unique_code'], registration_ids)
...
async def main_loop():
    while True:
        print("检查是否有待推送的新闻...")
        process_messages()
        await asyncio.sleep(30)  # 间隔 30 秒再次检查
...
if __name__ == "__main__":
    ...
    uniqCode = 'aabb'
    registration_ids = ["171976fa8ba39a121c0"]
    audience_registration_ids(title, content, uniqCode, registration_ids)
    quit()
