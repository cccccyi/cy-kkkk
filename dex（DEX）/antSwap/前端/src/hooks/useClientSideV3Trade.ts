...
export function useClientSideV3Trade<TTradeType extends TradeType>(
  ...
  const [currencyIn, currencyOut] =
    ...
    console.log("133 [currencyIn/out]", { currencyIn, currencyOut })

  const { routes, loading: routesLoading } = useAllV3Routes(currencyIn, currencyOut)
console.log("133 [routes]", { routes, routesLoading })

  const { chainId } = useWeb3React()
  console.log("133 [chainId from web3react]", chainId)
...
console.log("133 [quotesResults]", quotesResults)
  ...
  return useMemo(() => {
    ...
    console.log("133 [bestRoute result]", { bestRoute, amountIn, amountOut })
