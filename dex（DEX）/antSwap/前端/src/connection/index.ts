...
import OKX_ICON from 'assets/images/okx.webp'
...
//新增 OKXWallet 类,用于链接okx钱包
class OKXWalletConnector extends Connector {
  ...
  private setupListeners(provider: any) {
    // 链切换
...
const [web3Network, web3NetworkHooks] = initializeConnector<Network>(
  (actions) => new Network({ actions, urlMap: RPC_PROVIDERS, defaultChainId: 177 })
)
...
const [web3CoinbaseWallet, web3CoinbaseWalletHooks] = initializeConnector<CoinbaseWallet>(
  (actions) =>
    new CoinbaseWallet({
      actions,
      options: {
        url: RPC_URLS[SupportedChainId.HASHKEY][0],
        appName: 'Uniswap',
...
const [web3OkxWallet, web3OkxWalletHooks] = initializeConnector<OKXWalletConnector>(
  (actions) => new OKXWalletConnector(actions, onError)
)
...
const okxWalletConnection: Connection = {
  ...
  getIcon: () => OKX_ICON,
  shouldDisplay: () => typeof window !== 'undefined' && !!(window as any).okxwallet,
}
