...
//   console.log('币id'+currencyId);
//   console.log('币对象'+JSON.stringify(tokens));
//   console.log('转换后币名称',nativeOnChain(177));
...
function getCurrency(currencyId: string, chainId: SupportedChainId, tokens: TokenAddressMap): Currency | undefined {
  console.group(`🔍 调试 getCurrency`);
  console.log('链 ID:', chainId);
  console.log('币 ID:', currencyId);
  ...
  if (isNativeETH) {
    ...
    console.log('🟢 识别为原生币:', native.symbol);
    console.groupEnd();
    return native;
  }
  ...
  if (!chainTokens) {
    console.warn('⚠️ 未找到该链的代币列表:', chainId);
    console.groupEnd();
    return undefined;
  }
  ...
  const tokenEntry = chainTokens[currencyId] || chainTokens[lowerId];

  if (tokenEntry) {
    console.log('✅ 找到代币对象:', tokenEntry.token?.symbol || '无 symbol', tokenEntry.token);
  } else {
    console.warn('❌ 未找到代币:', currencyId);
    console.log('📋 当前链代币 keys (前10个):', Object.keys(chainTokens).slice(0, 10));
  }
...
function buildCurrencyDescriptor(
  ...
  // 为缺失的代币显示代币ID的前6位字符
  const formattedA = currencyA ? formatCurrencyAmount(CurrencyAmount.fromRawAmount(currencyA, amtA)) : t`Unknown`;
  const symbolA = currencyA?.symbol ?? (currencyIdA ? `(${currencyIdA.slice(0, 4)}...${currencyIdA.slice(-4)})` : '');
  
  const formattedB = currencyB ? formatCurrencyAmount(CurrencyAmount.fromRawAmount(currencyB, amtB)) : t`Unknown`;
  const symbolB = currencyB?.symbol ?? (currencyIdB ? `(${currencyIdB.slice(0, 4)}...${currencyIdB.slice(-4)})` : '');
...
function parseSwap(
  ...
  return {
    descriptor: buildCurrencyDescriptor(tokenIn, inputRaw, tokenOut, outputRaw, t`for`, swap.inputCurrencyId, swap.outputCurrencyId),
    currencies: [tokenIn, tokenOut],
  };
}
