...
export default function WalletModal({ openSettings }: { openSettings: () => void }) {
  ...
  const { connector, account, chainId: walletChainId, provider } = useWeb3React()
  ...
  // 获取系统当前选择的链ID
  const systemChainId = useAppSelector((state) => state.application.chainId)
  ...
  // useEffect(() => {
  //   if (systemChainId && isSupportedChain(systemChainId) && connector !== networkConnection.connector) {
  //     networkConnection.connector.activate(systemChainId)
  //   }
  // }, [systemChainId, connector])
  ...
  useEffect(() => {
    if (account && account !== lastActiveWalletAddress) {
      ...
      // 将null转换为undefined以匹配函数参数类型
      sendAnalyticsEventAndUserInfo(account, walletName, systemChainId ?? undefined, isReconnect, peerWalletAgent)
  ...
  const tryActivation = useCallback(
    async (connection: Connection) => {
      ...
      try {
        ...
        // 关键修改：钱包连接成功后，获取系统当前链ID，然后让钱包切换到这个链
        if (systemChainId && isSupportedChain(systemChainId)) {
          try {
            await switchChain(connection.connector, systemChainId)
          } catch (error) {
            console.error('Failed to switch wallet to system chain', error)
            // 这里可以添加错误处理，比如显示提示信息，但不应该阻止钱包连接
          }
        }
    ...
    [dispatch, setPendingError, toggleWalletDrawer, systemChainId]
  )
