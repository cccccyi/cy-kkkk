...
import bluelight from 'assets/images/bluelight.png'
    ...
    <Trace page={InterfacePageName.SWAP_PAGE} shouldLogImpression>
      <>
        ...
        {/* <img
          src={topLogo}
          alt="Top Logo"
          style={{ width: '210px', height: 'auto', margin: '70px auto 0 auto', display: 'block', position: 'relative', zIndex: '1' }}
        /> */}
        <TopLogoImg src={topLogo} alt="Top Logo" />
        {/* <img
            src={bluelight}
            alt="Top Logo" 
            style={{ width: '380px', height: 'auto', margin: '0 auto', display: 'block',position:'relative',bottom:'-21px' }} 
          /> */}
        {/*  */}
        <PageWrapper>
          <BlueLightImg
        ...
        <LearnMoreContainer
          onClick={() => {
            if (cardsRef.current) {
              const top = cardsRef.current.getBoundingClientRect().top + window.scrollY
              window.scrollTo({
                top: top - 70, // 👈 这里调整偏移量，比如留出 100px
                behavior: 'smooth',
              })
            }
          }}
        ...
        <AboutContentContainer isDarkMode>
          <CardGrid cols={3} ref={cardsRef}>
            {MORE_CARDS.map(({ darkIcon, lightIcon, ...card }) => (
              <Card {...card} icon={darkIcon} key={card.title} type={CardType.Secondary} />
            ))}
          </CardGrid>
          <AboutFooter />
