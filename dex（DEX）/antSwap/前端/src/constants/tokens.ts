...
export const USDC_HSK = new Token(
  SupportedChainId.HASHKEY,
...
export const USDT_HSK = new Token(
  SupportedChainId.HASHKEY,
...
export const WRAPPED_NATIVE_CURRENCY: { [chainId: number]: Token | undefined } = {
  ...
  [SupportedChainId.HASHKEY]: new Token(
    SupportedChainId.HASHKEY,
...
// 新增hashkey testnet识别函数
function isHashkey(chainId: number): chainId is SupportedChainId.HASHKEY {
  return chainId === SupportedChainId.HASHKEY;
}
...
class HashkeyNativeCurrency extends NativeCurrency {
  ...
  get wrapped(): Token {
    // 调试输出
    if (!isHashkey(this.chainId)) throw new Error('Not hashkey');
  ...
  public constructor(chainId: number) {
    if (!isHashkey(chainId)) throw new Error('Not hashkey');
    super(chainId, 18, 'HSK', 'Hashkey');
  }
}
// class HashkeyNativeCurrency extends NativeCurrency {
//   equals(other: Currency): boolean {
...
//   public constructor(chainId: number) {
//     if (!isHashkey(chainId)) throw new Error('Not hashkey');
//     super(chainId, 18, 'HSK', 'Hashkey Token');
//   }
...
class ExtendedEther extends Ether {
  public get wrapped(): Token {
    ...
    throw new Error(`Unsupported1 chain I1D: ${this.chainId}`)
  }
...
export function nativeOnChain(chainId: number): NativeCurrency | Token {
  ...
  } else if (isHashkey(chainId)) {
    nativeCurrency = new HashkeyNativeCurrency(chainId);
  } else {
...
export const TOKEN_SHORTHANDS: { [shorthand: string]: { [chainId in SupportedChainId]?: string } } = {
  USDC: {
    ...
    [SupportedChainId.HASHKEY]: USDC_HSK.address,
