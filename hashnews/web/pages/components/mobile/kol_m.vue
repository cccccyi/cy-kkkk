      ...
      <!-- <ClientOnly> -->
        ...
        <div  v-for="item,index in xList" class="newsItem setBox " @click="goDt(item.tweetId)" style="border: none !important;">
          ...
          <div class="flex1" style="padding:0px 0 20px 0;">
            <div class="newsTitle">
              <span class="cctime">{{formatDate(item.createdAt)}}</span>  <font style="color: rgb(163, 162, 162);position: relative;top:-1px">@{{item.screenName}}</font>
              <br>
            ...
            <div v-if="item.mediaUrlHttpsJson.length > 1" style="overflow: hidden;width:93%;margin-bottom: 20px;" >
            ...
            <p class="timeW">
              ...
              <span class="toX" >
                  <font-awesome-icon :icon="['fas', 'copy']" class="flIcon " @click.stop="copy(item['fullText'])" />
              </span>
            </p>
          </div>
        </div>
...
<script setup lang="ts">
  ...
  const goDt = (tid: any) => {
    //  navigateTo('newsx?uid=' + uid+'&tid='+tid);  // 使用router.push进行路由跳转
     window.open('x_kol?tid=' + tid+'&t=whale');  // 使用router.push进行路由跳转
  }
