...
function checkTaskFinished() {
  ...
  if ($g.isFailure || allActions.length <= $g.currentStep || $g.setTaskFinished) {
    ...
    if (netWorkParseFlag && taskConfig.networkSwitch === true) {
      netWorkParseFlag = false
      try {
        $g.resultsEx = {}
...
async function startExec() {
  $g.echoMsg("start_exec: step count: " + allActions.length);
  //let netWorkParseFlag = true
  var execTag = setInterval(async function () {
    ...
    if ($g.classicMode && $g.processExecTag &&
      ...
      $g.acts.ignoreAll("action timeout");
...
async function isBlankContent(pos) {
  ...
  const content = await $g.domOp.getHTML();
  ...
  try {
    ...
    mainWindow.webContents.debugger.on('message', (event, method, params) => {
      ...
      if (method === 'Network.responseReceived' && filterRequestTypes.indexOf(params.type) > -1) {
        ...
        // console.log('Event: responseReceived ' + requestId + '-' + params.response.url)
      }
      if (method === 'Network.loadingFinished') {
        if (networkDataIndex.hasOwnProperty(requestId)) {
          if (networkDataIndex[requestId].type == 'XHR') {
            mainWindow.webContents.debugger.sendCommand('Network.getRequestPostData', {
              requestId
            }).then(
              response => {
...
exports.getTaskConfig = () => {
  return {
    loadImages: taskConfig.loadImages
  }
}
// --userData
// --cookieFile
exports.getCmdArgument = (argName) => {
  return $g.common.arg.getArg(argName)
}
...
exports.execScript = async (mainWindow) => {
  ...
  try {
    ...
    //net workd
    if (taskConfig.networkSwitch === true) {
