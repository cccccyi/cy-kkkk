[{
    ...
    "name": "open",
    "params": {
        "url": "https://www.facebook.com/search/top/?q={{searchKeyword}}"
    }
},{
    "name": "wait",
    "params": {
    ...
    "params": {
        "checkers": [{
            "type": "checkText",
            "value": "{{skipSearchKeywordPost}}",
            "expected": "y",
            "operator": "equal"
        }],
        "label": "skipSearchKeywordPost"
    }
},{
    "name": "call",
    "params": {
    ...
    "params": {
        "checkers": [{
            "type": "checkText",
            "value": "{{skipSearchKeywordPage}}",
            "expected": "y",
            "operator": "equal"
        }],
        "label": "skipSearchKeywordPage"
    }
},{
    "name": "call",
    "params": {
    ...
    "params": {
        "checkers": [{
            "type": "checkText",
            "value": "{{skipSearchKeywordGroup}}",
            "expected": "y",
            "operator": "equal"
        }],
        "label": "skipSearchKeywordGroup"
    }
},{
    "name": "call",
    "params": {
    ...
    "name": "call",
    "params": {
        "code": "try{ document.querySelector(\"input[aria-label='Public Groups']\").click() }catch{}"
    }
},{
    "name": "loop",
    "loop_count": "{{monitorFbPostsLoopCount}}",
    ...
    "params": {
        "checkers": [{
            "type": "checkText",
            "value": "{{skipSearchKeywordPeople}}",
            "expected": "y",
            "operator": "equal"
        }],
        "label": "skipSearchKeywordPeople"
    }
},{
    "name": "call",
    "params": {
    ...
    "params": {
        "checkers": [{
            "type": "checkText",
            "value": "{{skipSearchKeywordPlace}}",
            "expected": "y",
            "operator": "equal"
        }],
        "label": "skipSearchKeywordPlace"
    }
},{
    "name": "call",
    "params": {
    ...
    "name": "call",
    "params": {
        "code": "const sleep=ms=>{return new Promise(resolve => setTimeout(resolve, ms))}; const run_call=async function(){ const btn_more=document.evaluate(\"//div[@aria-haspopup='menu']//span[text()='More']\",document).iterateNext();if(btn_more){btn_more.click()}else{return;} await sleep(2000); const btn_event=document.querySelector(\"a[href$='events']\");if(btn_event){btn_event.click()}else{return;} await sleep(2000); for(let i=0;i<5;i++){ await sleep(2000); window.scrollTo(0, 100000); }  window.scrollTo(0,120);await sleep(1000);btn_event.scrollIntoView(true);await sleep(1000);console.log(btn_more); btn=document.querySelector(\"a[href*='past_hosted_events']\"); console.log(btn); if(btn){ btn.click();for(i=0; i<5;i++){ await sleep(2000); window.scrollTo(0, 100000); } } };  run_call(); "
    }
},{
    "name": "data",
    "params": {
        "name": "pastEventUrlList",
        "blocks": ["span>div>a[href*='/events/']"],
        "fields": {
            "url": {
                "target": "<self>",
                "from": "href",
                "variables": "past_event_url_list"
            }
        }
    }
},{
    "name": "call",
    "params": {
        "code": "try{ const sleep=ms=>{return new Promise(resolve => setTimeout(resolve, ms))}; const run_call=async function(){  const event_btn=document.querySelector(\"a[href$='/events/']\"); if(event_btn) {event_btn.click();}else{return;} await sleep(2000); let index=0; let btn=null; while(btn=document.evaluate(\"//span[text()='See More' or text()='See more']\", document).iterateNext()){ btn.click();await sleep(2000);index++;if(index>10){break;}} };  run_call(); }catch{}"  
    }
},{
    "name": "loop",
