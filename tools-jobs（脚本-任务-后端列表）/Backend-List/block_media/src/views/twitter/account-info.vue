    ...
    >
      <el-table-column label="screen_name">
        <template slot-scope="{row}">
          ...
          <el-tag type="success" v-if="row.has_photo=='y'">有头像</el-tag>&nbsp;
          <el-tag type="danger" v-else>无头像</el-tag>&nbsp;
        </template>
      </el-table-column>
      ...
      <el-table-column label="账号" width="180px">
        <template slot-scope="{row}">
          <span>{{ row.name }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="引导信息" width="200px">
        <template slot-scope="{row}">
          <span title="imei/layer/active,error_code">{{ row.device_imei + ' / ' + row.bind_layer + ' / ' + row.active + ' / ' + row.error_code }}</span>
        </template>
      </el-table-column>
      <el-table-column label="判死时间" width="150">
        <template slot-scope="{row}">
      ...
      <el-table-column label="最新贴" width="150">
        <template slot-scope="{row}">
          <el-link v-if="row.latest_post_time" :href="`https://twitter.com/${row.screen_name}/status/${row.tweet_id}`" target="_blank" type="primary">{{ row.latest_post_time}}</el-link>
        </template>
      </el-table-column>
      <el-table-column label="采集" width="60">
        <template slot-scope="{row}">
          <span>{{ row.crawl_available + ' ' + row.profile_interstitial_type }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog title="导入 引导Twitter账号 信息" :visible.sync="dialogUploadFormVisible">
      <div style="text-align:center;">
        <el-upload
          ref="uploadFile"
          drag
          action="http://221.120.163.66:38011/boost_interface/vue-element-admin/twitter/uploadTwAccountInfosDataYY"
          :multiple="false"
          :limit="1"
          accept=".xls, .xlsx"
          :auto-upload="false"
          ...
          <div class="el-upload__tip" slot="tip">
            ...
            EXCEL列定义： screen_name, place_id, imei, bind_layer, active, error_code, dead_time
          </div>
        </el-upload>
      </div>
...
<script>
import { fetchGuidTwAccountListYY } from '@/api/twitter'
...
export default {
  ...
  methods: {
    getList() {
      ...
      this.list = []
      fetchGuidTwAccountListYY(this.listQuery).then(response => {
    ...
    handleExport() {
      ...
      const url = `http://221.120.163.66:38011/boost_interface/vue-element-admin/twitter/exportAccountInfosYY?${params.join('&')}`
