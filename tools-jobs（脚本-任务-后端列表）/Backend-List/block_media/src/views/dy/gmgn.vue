<template>
  <div class="app-container">
    <div class="filter-container">
      <el-select v-model="listQuery.chain" placeholder="所在链" clearable class="filter-item" style="width: 130px">
        <el-option v-for="item in chainOptions" :key="item.key" :label="item.label" :value="item.key" />
      </el-select>&nbsp;
    ...
    >
      ...
      <el-table-column label="所在链" width="">
        <template slot-scope="{row}">
          <span>{{ row.chain }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="流动性" width="">
        <template slot-scope="{row}">
          <span>{{ row.liquidity }}</span>
        </template>
      </el-table-column>
      <el-table-column label="持有人" width="">
        <template slot-scope="{row}">
          <span>{{ row.holder_count }}</span>
        </template>
      </el-table-column>
...
<script>
import { fetchGmgnCoinList, importGmgnData, updateConfig } from '@/api/douyin'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchGmgnCoinList(this.listQuery).then(response => {
