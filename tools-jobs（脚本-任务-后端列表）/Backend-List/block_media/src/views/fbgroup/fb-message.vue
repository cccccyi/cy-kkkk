<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="发送方" width="150px">
        <template slot-scope="{row}">
          <el-link :href="`https://www.facebook.com/${row.send_fid}`" target="_blank" type="primary">{{ row.a_id + ',' +row.account }}</el-link>
        </template>
      </el-table-column>
      <el-table-column label="接收方">
        <template slot-scope="{row}">
          <el-link :href="`https://www.facebook.com/${row.to_fid}`" target="_blank" type="primary">{{ row.name}}</el-link>
        </template>
      </el-table-column>
...
<script>
import { fetchMessageList } from '@/api/group'
...
<style scoped>
.task-screenshot-image img {
  width: 200px;
}
.task-screenshot-image img:hover{
  position: absolute;
  top: 100px;
  left: 100px;
  width: 800px;
