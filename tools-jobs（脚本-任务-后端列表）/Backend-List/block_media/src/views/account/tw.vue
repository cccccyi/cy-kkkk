<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="screen_name" width="150px">
        <template slot-scope="{row}">
          <el-link :href="row.account_url" target="_blank" type="primary">{{ row.screen_name}}</el-link>&nbsp;
          <el-tag :type="row.available | statusFilter"> {{ row.available }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="exists" width="80px">
        <template slot-scope="{row}">
          <span>{{ row.account_exists }}</span>
        </template>
      </el-table-column>
      <el-table-column label="Profile" width="100px">
        <template slot-scope="{row}">
      ...
      <el-table-column label="Cover" width="100px">
        <template slot-scope="{row}">
          <el-image v-if="row.banner_picture" style="width:25px;height:25px;vertical-align:middle;" :src="row.banner_picture" ></el-image>
        </template>
      </el-table-column>
      ...
      <el-table-column label="followers_count" width="120px">
        <template slot-scope="{row}">
          <span>{{ row.followers_count }}</span>
        </template>
      </el-table-column>
      <el-table-column label="following_count" width="120px">
        <template slot-scope="{row}">
          <span>{{ row.following_count }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="采集时间" prop="last_update_time" width="120px">
        <template slot-scope="{row}">
    ...
    <pagination v-show="total>0" :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.limit" @pagination="getList" :page-sizes="[100, 200]" />
...
<script>
import { fetchTwAccountList } from '@/api/account'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchTwAccountList(this.listQuery).then(response => {
