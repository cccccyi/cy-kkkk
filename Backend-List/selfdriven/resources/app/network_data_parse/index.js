...
const getParseFunName = function(url, requestType, postData) {
    ...
    if (url.indexOf('facebook.com') > -1) {
        ...
        if(requestType == 'Document'){
            ...
            if(url.indexOf('UserByScreenName') > -1){
                return 'twUserInfoParse'
                    ...
                    url.indexOf('UserTweetsAndReplies') > -1 || 
            ...
            }else if (url.indexOf('ListMembers') > -1){
                return 'twListMembersParse'
            }else if (url.indexOf('ListSubscribers') > -1){
                return 'twListSubscribers'
            }else if (url.indexOf('membersSliceTimeline_Query') > -1){
                return 'membersSliceTimeline_Query'
            }else if (url.indexOf('dm/inbox_initial_state.json') > -1 || url.indexOf('dm/inbox_timeline') > -1){
                return 'dmMsgList'
            }else if (url.indexOf('CreateTweet') > -1 || url.indexOf('CreateNoteTweet') > -1){
                return 'twCreateTweet'
            }else if (url.indexOf('ListAddMember') > -1){
                return 'twListAddMember'
            }else if (url.indexOf('CommunitiesSearchQuery') > -1){
                return 'twCommunitiesSearchQuery';
            }
        }
        ...
        if (requestType == 'Document') {
            ...
            if(url.indexOf('statuses/update') > -1){
                return 'createWeibo';
            }
        }
    ...
    }else if(url.indexOf('api.panewslab.com') > -1){
        if(requestType == 'Document'){

        }else{
            if(url.indexOf('webapi/flashnews') > -1){
                return 'flashnews'
            }
        }
    ...
    }else if(url.indexOf('cointelegraph.com') > -1){
        if(requestType == 'Document'){
            if(url.indexOf('/category/latest-news') > -1 ||
               url.indexOf('/category/opinion') > -1){
                return 'latestNews'
            }else if(url.indexOf('/news/') > -1){
                return 'coinTelegraphNewDetail'
            }
        }
        ...
        if(requestType == 'Document'){
            ...
            }else if(url.indexOf('/square/fear-and-greed-index') > -1){
                return 'bianceFearAndGreedIndex'
            }
        }else{
    ...
    }else if(url.indexOf('okx.com') > -1){
        console.log(url)
        if(requestType == 'Document'){
            console.log(url)
            if(url.indexOf('section/announcements-new-listings') > -1){
                return 'okxAnnouncementList'
            }
        }
    }else if(url.indexOf('api.coinank.com') > -1){
        if(requestType == 'Document'){

        }else{
            if(url.indexOf('longShortRatio') > -1){
                return 'longShortRatio'
            }else if(url.indexOf('api/Statistics/all') > -1){
                return 'statisticsAll'
            }else if(url.indexOf('liquidation/statisticV2') > -1){
                return 'liquidationStatisticV2'
            }else if(url.indexOf('getAltcoinSeasonMini?limit=1') > -1){
                return 'getAltcoinSeasonMini'
            }
        }
    }else if(url.indexOf('capi.coinglass.com') > -1){
        if(requestType == 'Document'){

        }else{
            //if(url === 'https://capi.coinglass.com/api/hyperliquid/topPosition'){
            if(url.indexOf('api/hyperliquid/topPosition') > -1){
                return 'topPosition'
            }
        }
    }else if(url == 'https://api-ui.hyperliquid.xyz/info'){
        return 'hyperliquidInfo'
    }else if(url.indexOf('okcis.cn') > -1){
        if(requestType == 'Document'){

        }else{
            if(url.indexOf('/zhao_biao/gong_gao/listajax.php') > -1){
                return 'zhaoBiaoGongGao'
            }
        }
    }else if(url === 'websocket'){
        return 'websocketParse'
    }
    return false
}
