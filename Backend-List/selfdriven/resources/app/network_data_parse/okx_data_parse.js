...
exports.okxAnnouncementList = function(htmlStr, requestData){
    const pattern = /<script.*?id="appState">(.*?})<\/script>/
    const matchObj = htmlStr.match(pattern)
    $g.log.append('debug okxAnnouncementDetail 1', JSON.stringify(matchObj))
    let news = []
    if(matchObj && matchObj[1]){
        $g.log.append('debug okxAnnouncementDetail 1', matchObj[1])
    }
    return
    //$g.log.append('debug okxAnnouncementList', htmlStr)
...
exports.okxAnnouncementDetail = function(htmlStr, requestData){
