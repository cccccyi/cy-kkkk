...
export class RpcService {
  ...
  public async sendRawTransaction(rawTransactionHex: string, logException: boolean = true, throwException: boolean = false) {
    const now = Date.now();
    const rpcData = {
      jsonrpc: '1.0',
      id: now,
      method: 'sendrawtransaction',
      params: [rawTransactionHex], // sendrawtransaction 需要的参数
    };
    return this.rpc(rpcData, logException, throwException);
  }

  public async getRawTransaction(txId: string, logException: boolean = true, throwException: boolean = false) {
    const now = Date.now();
    const rpcData = {
      jsonrpc: '1.0',
      id: now,
      method: 'getrawtransaction', // 使用 getrawtransaction 方法
      params: [txId, true],
    };
    return this.rpc(rpcData, logException, throwException);
  }
