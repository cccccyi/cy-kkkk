...
export function ownerAddressToPubKeyHash(ownerAddr: string) {
  try {
    ...
    if (pubKey) {
      return Buffer.from(crypto.hash160(Buffer.from(pubKey, 'hex'))).toString('hex');
    }
    ...
    if (match && match[1] && match[2]) {
      switch (match[1]) {
        case EnvelopeMarker.Token:
          ...
          if (metadata && metadata['name'] !== undefined && metadata['symbol'] !== undefined && metadata['decimals'] !== undefined) {
            return {
