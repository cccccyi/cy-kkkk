...
import { ConsoleLogger, ExecutionContext, ForbiddenException, Inject, Injectable, UnauthorizedException } from '@nestjs/common';
  ...
  checkWhiteList(ctx: ExecutionContext): boolean {
    ...
    const i = this.globalWhiteList.findIndex((route) => {
      ...
      if (!route.method || req.method.toUpperCase() === route.method.toUpperCase()) {
        // 对比 url
        // console.log('pathToRegexp:', route.path, req.url, pathToRegexp(route.path));
