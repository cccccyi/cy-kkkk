  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="UID">
        <template slot-scope="{row}">
          <el-link :href="'https://www.uwants.com/space.php?action=viewpro&view=home_view&uid=' + row.uid" target="_blank" type="primary">{{ row.uid}}</el-link>
        </template>
      </el-table-column>
      ...
      <el-table-column label="帖子 / 追贴 / 粉丝">
        <template slot-scope="{row}">
          <span>{{ row.thread_count + ' / ' + row.follow_thread_count + ' / ' + row.follower_count }}</span>
        </template>
      </el-table-column>
...
<script>
import { fetchAccountList, createAccount, updateAccount } from '@/api/uwants'
