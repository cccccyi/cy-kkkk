<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="贴吧ID">
        <template slot-scope="{row}">
          <span>{{ row.user_id }}</span>
        </template>
      </el-table-column>
      <el-table-column label="浏览器" width="60px">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        <el-form-item label="账号" prop="identity">
          <el-input v-model="temp.account" placeholder="输入贴吧账号" />
        </el-form-item>
        <el-form-item label="密码" prop="password">
          <el-input v-model="temp.password" placeholder="输入账号密码" />
        </el-form-item>
        <el-form-item label="贴吧ID" prop="user_id">
          <el-input v-model="temp.user_id" placeholder="输入账号密码" />
        </el-form-item>
        ...
        <el-form-item label="Cookie" prop="text">
          <el-input v-model="temp.cookie_str" :autosize="{ minRows: 2, maxRows: 4}" type="textarea" placeholder="输入Cookie，可以在浏览器拷贝" />
        </el-form-item>
      </el-form>
...
<script>
import { fetchTiebaAccountList, createTiebaAccount, updateTiebaAccount } from '@/api/account'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        cookie_str: [{ required: true, message: '账号Cookie', trigger: 'change' }]
      },
  ...
  methods: {
    getList() {
      ...
      fetchTiebaAccountList(this.listQuery).then(response => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
            createTiebaAccount(this.temp).then(() => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateTiebaAccount(tempData).then(() => {
