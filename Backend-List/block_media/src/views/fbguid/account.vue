<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-input v-model="listQuery.loc" placeholder="账号Loc" clearable class="filter-item" style="width:120px" />&nbsp;
      <el-input v-model="listQuery.tag" placeholder="账号Tag" clearable class="filter-item" style="width:120px" />&nbsp;
      <!--
    ...
    >
      ...
      <el-table-column label="账号">
        <template slot-scope="{row}">
          ...
          <el-tag type="success" v-if="row.available=='y'">存活：{{ row.available }}</el-tag>&nbsp;
          <el-tag type="danger" v-if="row.available=='n'">存活：{{ row.available }}</el-tag>&nbsp;
          <el-tag type="warning" v-if="row.disable_date">禁用时间：{{ row.disable_date }}</el-tag>&nbsp;
        </template>
      </el-table-column>
      ...
      <el-table-column label="标签" width="120px">
        <template slot-scope="{row}">
          <el-tag type="warning" v-if="row.tag">{{ row.tag }}</el-tag>&nbsp;
        </template>
      </el-table-column>
      <!--
      ...
      -->
      <el-table-column label="操作时间" prop="last_use_time" width="120px" sortable="custom">
        <template slot-scope="{row}">
          <span>{{ row.last_use_time }}</span>
        </template>
      </el-table-column>
      <!--
      <el-table-column label="帖子列表" prop="last_crawl_manage_pages" width="120px" sortable="custom">
        <template slot-scope="{row}">
          <span>{{ row.last_crawl_manage_pages }}</span>
        </template>
      </el-table-column>
      -->
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 500px; margin-left:50px;">
        <el-form-item label="FID" prop="account_fid">
          <el-input v-model="temp.account_fid" placeholder="输入账号FID" />
        </el-form-item>
        ...
        <el-form-item label="Tag" prop="tag">
          <el-input v-model="temp.tag" placeholder="为账号添加标签，区分不同用途" />
        </el-form-item>
        <el-form-item label="名称" prop="account">
          <el-input v-model="temp.account" placeholder="账号名称，采集主页相关数据是，需要根据名称选择切换" />
        </el-form-item>
...
<script>
import { fetchFbAccountListYY, createFbAccountYY, updateFbAccountYY, fetchAccountGroupListYY, fetchAccountFriendsListYY } from '@/api/account_yy'
...
export default {
  ...
  data() {
    return {
      ...
      prompt_text: '',
      rules: {
        account_fid: [{ required: true, message: '需要填写账号FID', trigger: 'blur' }],
        password: [{ required: true, message: '需要填写账号密码', trigger: 'blur' }],
        loc: [{ required: true, message: '需要填写所在设备', trigger: 'blur' }],
  ...
  methods: {
    ...
    getList() {
      ...
      fetchFbAccountListYY(this.listQuery).then(response => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createFbAccountYY(this.temp).then(() => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateFbAccountYY(tempData).then(() => {
            fullLoading.close()
            this.dialogFormVisible = false
            this.$notify({
              title: '成功',
    ...
    handleGetFriendRequestsListYY(identity) {
      this.friendRequestsListQuery.account_fid = identity
      this.getFriendRequestsList()
    },
    getFriendRequestsList() {
      fetchFriendRequestsListYY(this.friendRequestsListQuery).then(response => {
    ...
    getPymkList() {
      fetchPymkListYY(this.pymkListQuery).then(response => {
    ...
    handleFetchAccountGroup(account_fid) {
      this.listLoading = true
      fetchAccountGroupListYY({account_fid}).then(response => {
    ...
    getPymkActionList() {
      this.listLoading = true
      fetchAccountFriendsListYY(this.pymkActionListQuery).then(response => {
