<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="网站" width="120px">
        <template slot-scope="{row}">
          <span>{{ row.site }}</span>
        </template>
      </el-table-column>
      <el-table-column label="URL" width="400px">
        <template slot-scope="{row}">
          <span>{{ row.url }}</span>
        </template>
      </el-table-column>
      <el-table-column label="采集时间" width="150px">
        <template slot-scope="{row}">
          <span>{{ row.crawler_fetch_time_str }}</span>
        </template>
      </el-table-column>
      <el-table-column label="入库时间" width="200px">
        <template slot-scope="{row}">
          <span>{{ row.crawler_finish_time_str }}</span>&nbsp;
          <el-tag type="primary">{{ row.timeout }} 秒</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="备注">
        <template slot-scope="{row}">
          <span>{{ row.remark }}</span>&nbsp;
          <el-tag v-if="row.del_flag=='1'" type="danger">暂停</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width:600px; margin-left:50px;">
        <el-form-item label="网站" prop="site">
          <el-input v-model="temp.site" type="input" placeholder="采集目标，所在网站" />
        </el-form-item>
        <el-form-item label="URL" prop="url">
          <el-input v-model="temp.url" type="input" placeholder="采集 URL 链接" />
        </el-form-item>
...
<script>
import { fetchTargetList, createConfig, updateTarget } from '@/api/monitor'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchTargetList(this.listQuery).then(response => {
