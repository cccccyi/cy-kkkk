<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="密码" width="120px">
        <span>******</span>
      </el-table-column>
      <el-table-column label="分配手机">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        <el-form-item label="登录名" prop="name">
          <el-input v-model="temp.name" type="input" placeholder="输入专用浏览器登录名,唯一" />
        </el-form-item>
        <el-form-item label="密码" prop="password">
          <el-input v-model="temp.password" type="password" placeholder="输入密码" />
        </el-form-item>
        <el-form-item label="手机号">
          <el-input type="textarea" v-model="temp.phones" :rows="5" placeholder="为用户分配可接码手机号，多个英文逗号分隔"></el-input>
        </el-form-item>
      </el-form>
...
<script>
import { fetchUserList, createUser, updateUser } from '@/api/sms'
...
export default {
  ...
  data() {
    return {
      ...
      pvData: [],
      rules: {
        name: [{ required: true, message: '填写登录名', trigger: 'blur' }],
