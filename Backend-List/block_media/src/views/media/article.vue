    ...
    >
      ...
      <el-table-column label="快讯">
        <template slot-scope="{row}">
          ...
          <el-row style="font-weight:bold;color:orange;">简介</el-row>
          <el-row><p style="text-indent:20px;">{{ row.description }}</p></el-row>
          <div>
            <el-row style="font-weight:bold;color:orange;">内容</el-row>
      ...
      <el-table-column label="来源" width="120px">
        <template slot-scope="{row}">
          <div style="text-align:center;">
            ...
            <el-row><el-link :href="row.new_url" target="_blank" type="primary">{{ row.n_publish_time_str }}</el-link></el-row>
            <el-row>
              <el-image v-if="row.img" :src="row.img" :preview-src-list="[row.img]" referrer-policy="no-referrer" />
            </el-row>
          </div>
        </template>
      </el-table-column>
    ...
    <el-dialog v-loading="dialogLoading" :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 600px;">
        ...
        <el-form-item label="简介" prop="description">
          <el-input v-model="temp.description" :autosize="{ minRows: 3, maxRows: 5}" type="textarea" placeholder="简介" />
        </el-form-item>
        <el-form-item label="内容" prop="content">
          <el-input v-model="temp.content" :autosize="{ minRows: 5, maxRows: 10}" type="textarea" placeholder="内容" />
        </el-form-item>
        ...
        <el-form-item label="图片" prop="img">
          <el-upload
            class="upload-demo"
            action="http://82.157.161.88/boost_interface/vue-element-admin/hashNews/uploadImage"
...
<script>
import { fetchArticleList, createArticleInfo, updateArticleInfo } from '@/api/hashnews'
  ...
  data() {
    return {
      ...
      rules: {
        ...
        content: [{ required: true, message: '内容', trigger: 'change' }],
  ...
  methods: {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createArticleInfo(this.temp).then(() => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateArticleInfo(tempData).then((data) => {
            this.dialogFormVisible = false
            this.$notify({
              title: 'Success',
