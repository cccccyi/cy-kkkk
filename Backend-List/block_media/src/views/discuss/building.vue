<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="帖子链接/舆材">
        <template slot-scope="{row}">
          <el-link :href="row.thread_url" target="_blank" type="primary">{{ row.thread_url }}</el-link><br/>
          <span>{{ row.text }}</span>
        </template>
      </el-table-column>
      <el-table-column label="任务状态" width="240px">
        <template slot-scope="{row}">
          ...
          <span v-if="row.uid">使用账号：<el-link :href="'https://news.discuss.com.hk/space.php?uid=' + row.uid" target="_blank" type="primary">{{ row.uid}}</el-link></span><br/>
          <span>{{ '任务执行时间: ' + row.last_fetch_time }}</span><br/>
          <span v-if="row.post_time">发布时间：{{ row.post_time}}</span>
        </template>
      </el-table-column>
      <el-table-column label="截图" width="200px">
        <template slot-scope="{row}">
          <el-image v-if="row.screenshot_image" :src="'http://221.120.163.66:38011/uwants_images/' + row.screenshot_image" style="max-width: 180px;"></el-image>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 450px; margin-left:20px;">
        <el-form-item label="帖子链接" prop="thread_url">
          <el-input v-model="temp.thread_url" type="input" placeholder="请输入，目标贴链接" />
        </el-form-item>
...
<script>
import { fetchPostBuildingCorpusList, createPostBuildingCorpus } from '@/api/discuss'
...
export default {
  ...
  data() {
    return {
      ...
      pvData: [],
      rules: {
        thread_url: [{ required: true, message: 'url is required', trigger: 'change' }],
  ...
  methods: {
    getList() {
      ...
      this.list = []
      fetchPostBuildingCorpusList(this.listQuery).then(response => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createPostBuildingCorpus(this.temp).then(() => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          createPostBuildingCorpus(tempData).then(() => {
