<template>
  <div class="app-container defaultFont">
    <div class="filter-container">
      ...
      <el-input v-model="listQuery.tag" placeholder="标签" clearable class="filter-item" style="width:120px" />&nbsp;
    ...
    >
      <el-table-column label="名称">
        <template slot-scope="{row}">
          ...
          <el-tag type="success" v-if="row.page_available=='n'">存活：{{ row.page_available }}</el-tag>&nbsp;
          <template v-if="row.bio">
            <el-row>bio: {{ row.bio }}</el-row>
          </template>
        </template>
      </el-table-column>
      ...
      <el-table-column label="粉丝 / 点赞" prop="follower_count" width="120px" sortable="custom">
        <template slot-scope="{row}">
          <span>{{ row.followers_count + ' / ' + row.likes_count }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog title="导入 群组 信息" :visible.sync="dialogUploadFormVisible">
      <div style="text-align:center;">
        <el-upload
          ref="uploadFile"
          drag
          action="http://221.120.163.66:38011/boost_interface/vue-element-admin/page/uploadPageInfosDataYY"
          :multiple="false"
          :limit="1"
          accept=".xls, .xlsx"
          :auto-upload="false"
          ...
          <div class="el-upload__tip" slot="tip">
            只能上传xlsx文件，且不超过10M，一次只能上传一个，<br/>
            表格列定义： FID,URL,tag <br/>
            说明：EXCEL第一列为主页FID,第二列为主页URL,两个值需要存在一个才有效， tag列为数据打标方便查询与导出
          </div>
        </el-upload>
      </div>
...
<script>
import { fetchCollectPageListYY, updateCollectGroup } from '@/api/page'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchCollectPageListYY(this.listQuery).then(response => {
    ...
    handleExport() {
      ...
      const url = `http://221.120.163.66:38011/boost_interface/vue-element-admin/page/exportPageInfosYY?${params.join('&')}`
