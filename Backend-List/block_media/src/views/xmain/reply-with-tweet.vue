  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="链接" width="">
        <template slot-scope="{row}">
          <div><span>{{ row.url }}</span></div>
          <div v-if="row.tweet_post_time"><span>{{ '发帖时间：' + row.tweet_post_time }}</span></div>
          <div><span>{{ '更新时间：' + row.last_update_time }}</span></div>
          <div><el-tag type="danger">{{ '转发数：' + row.tweet_count }}</el-tag></div>
          <div><el-tag v-if="row.del_flag=='0'" type="success">有效</el-tag></div>
...
<script>
import { fetchReplyWithTweetConfigList, createReplyWithTweetConfig, updateReplyWithTweetConfig } from '@/api/keyword'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchReplyWithTweetConfigList(this.listQuery).then(response => {
