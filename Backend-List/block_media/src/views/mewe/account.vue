<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="真实头像" width="100px">
        <template slot-scope="{row}">
          <el-image v-if="row.photo" :src="row.photo" :key="img" style="max-width: 80px;"></el-image>
        </template>
      </el-table-column>
      <el-table-column label="设置头像" width="100px">
        <template slot-scope="{row}">
          <el-image v-if="row.upload_photo" :src="'http://221.120.163.66:38011/mewe_images/' + row.upload_photo" :key="img" style="max-width: 80px;"></el-image>
        </template>
      </el-table-column>
      ...
      <el-table-column label="@Name">
        <template slot-scope="{row}">
          <el-link :href="'https://mewe.com/' + row.screen_name" target="_blank" type="primary">{{ row.screen_name }}</el-link>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        <el-form-item label="邮箱" prop="email">
          <el-input v-model="temp.email" placeholder="输入Mewe账号, 邮箱" />
        </el-form-item>
        <el-form-item label="密码" prop="password">
          <el-input v-model="temp.password" placeholder="输入账号密码" />
        </el-form-item>
        <el-form-item label="@Name" prop="screen_name">
          <el-input v-model="temp.screen_name" placeholder="@Name" />
        </el-form-item>
...
<script>
import { fetchAccountList, createAccount, updateAccount } from '@/api/mewe'
...
export default {
  ...
  data() {
    return {
      ...
      pvData: [],
      rules: {
        email: [{ required: true, message: '需要填写账号email', trigger: 'blur' }],
