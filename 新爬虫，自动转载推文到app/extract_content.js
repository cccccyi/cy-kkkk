    ...
    if (tweetData) {
      ...
      if (!isArticleType) {
        ...
        try {
          ...
          const scrollDelay = 300; // 每次滚动后的等待时间（毫秒）
          ...
          await page.waitForTimeout(1000); // 额外等待一段时间确保所有内容加载完成
          
          console.log('✅ 滚动到页面底部完成');
        } catch (error) {
          console.error('⚠️  滚动到页面底部失败:', error);
        }
