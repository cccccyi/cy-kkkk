...
async function process_per_coin(){
  ...
  const [rows, fields] = await connection.execute("select id,slug,update_time from dt_crawl_cmc_coin_list order by id limit 30")
...
async function process_per_exchange(){
  ...
  const [rows, fields] = await connection.execute("select id,slug,dexStatus,type,spotVol24h,derivativesVol24h from dt_crawl_exchange_list where rank is not null order by rank limit 10");
...
async function process_per_exchange_dex_spot(){
  // const [rows, fields] = await connection.execute("select id,slug,dexStatus,type,spotVol24h,derivativesVol24h from dt_crawl_exchange_list where id=6706 order by rank");
  const [rows, fields] = await connection.execute("select id,slug,platformId from dt_crawl_exchange_list where rank is not null order by rank limit 30");
  for(const row of rows){
      const slug = row.slug
